#!/bin/bash
# port.sh v5.5 — 代理节点防火墙管理脚本（nftables 版）
...
#
# 【Docker 集成】
#   不修改 daemon.json，Docker iptables-nft（ip族）与本脚本（inet族）共存
#   动态读取所有 Docker 网络子网（IPv4+IPv6），生成对应 masquerade 规则
#   -p 映射端口由 docker-proxy 监听，ss 扫描自动放行，无需额外 DNAT
#
# 【原子加载】
#   规则写入临时文件后 nft -f 一次性加载，无规则空窗期
#   save_rules 将 build_nft_ruleset 输出写到 /etc/nftables.conf 持久化
#
# ══════════════════ v5.5 修复（Gemini 架构改进）══════════════════════════════
# [G1] apply_rules: 消除防御真空期与 nft -c 失败后裸奔风险（架构改进）
#      原因（双重风险）：
#        ① 裸奔：bash 层先 nft delete → nft -c 失败 → err() 退出 → 两表已删，裸奔
#        ② 真空期：bash delete 到 nft -f 之间毫秒级窗口无任何防火墙规则
#      修复：将删表操作移入 .nft 文件，使用 nftables 原生原子模式：
#              table inet filter {}      ← 幂等 add（首次建表/已有不报错）
#              delete table inet filter  ← 删旧表
#              table inet filter { ... } ← 重建完整规则
#            nft -c 只做语法解析不执行任何操作；
#            nft -f 将整个文件通过 nfnl_batch_begin/end 作为单一内核事务提交，
#            语法错误时内核回滚，旧规则完整保留；加载成功时切换无中间状态
# ══════════════════ v5.4 修复（Gemini 报告，均为真实 Bug）══════════════════════
# [F1] build_nft_ruleset / reset_fw: Shell 语法混入 .nft 文件（致命）
#      原因：delete table ... 2>/dev/null || true 写进 heredoc 输出的 .nft 文件，
#            nft 解析器遇到 2>/dev/null 直接报 "syntax error" 终止加载，
#            导致每次运行和每次开机 nftables.service 都必定失败
#      修复：.nft 文件只含纯 nftables 语法；删表操作移到 bash 层（apply_rules
#            调用 nft -f 之前），reset_fw 生成的 nftables.conf 只写注释
# [F2] forward chain: policy drop 阻断 Docker -p 端口映射（致命）
#      原因：Docker DNAT 后包走 FORWARD 而非 INPUT，policy drop 将 -p 映射
#            的所有新入站连接全部丢弃，容器公网端口完全不通
#      修复：改为 policy accept，由 Docker 自身的 iptables-nft（ip filter）
#            规则管理容器安全；inet filter INPUT 的 policy drop 继续保护本机
#      补充：chain forward 开头加 ct state invalid drop（过滤伪造包）
# [F3] add_hop_interactive: 步骤4 冗余 Docker 检测（瑕疵）
#      原因：手动设 _DOCKER_RUNNING=1，但 apply_rules 首行会无条件
#            将其重置为0再重新检测，步骤4 的赋值被完全覆盖
#      修复：删除冗余步骤，注释说明 apply_rules 内部已自行处理
# ══════════════════ v5.3 修复（GPT 第二轮报告分析）══════════════════════════
# [E1] _get_docker_bridges ip link 正则：[0-9a-f]+ → [0-9a-fA-F]+
#      原因：Docker 偶发生成含大写 hex 的接口名（如 br-A1B2C3D4E5F6）
#      修复：改用 ip -o + awk + cut 方案，支持大小写 hex，同时去掉 @ifN 后缀
# [E2] install_deps 新增内核版本提前检测（≥ 5.2 才支持 inet NAT）
#      原因：内核过低时 nft -c/-f 报错信息难以理解
#      修复：uname -r 解析主次版本号，< 5.2 时 err() 退出并给出明确提示
# ──────────────────────────────────────────────────────────────────────────
# GPT 第二轮误判（已核实，不修改）：
#   • set -e：nft -f 在 if 块内，mktemp 失败立即可见，已有充分结构保护
#   • IPv6正则 fd00::/64：实测已正确匹配（两轮均测试通过，GPT连续误判）
#   • add_port 数组匹配：" $p " 两侧空格保证精确匹配，测试无误匹配
#   • _cleanup && || true：逻辑正确，优先级无歧义
# ══════════════════ v5.2 修复（GPT 报告分析）════════════════════════════════
# [D1] Docker forward 规则硬编码 docker0 → 动态检测所有 bridge 接口
#      原因：docker network create 产生 br-xxxx 接口，自定义网络容器流量被拦截
#      修复：新增 _get_docker_bridges()，双路检测（docker inspect + ip link），
#            build_nft_ruleset 为每个 bridge 接口生成独立的 forward accept 规则
# [D2] flush ruleset → delete table inet filter/nat（精准删除）
#      原因：flush ruleset 清空所有 nft 表，包括 fail2ban/k8s/其他工具的规则
#      修复：只删除本脚本管理的两张表（|| true 容错首次运行时表不存在）
#            reset_fw / nftables.conf 生成逻辑同步更新
# ──────────────────────────────────────────────────────────────────────────
# GPT 报告中误判的条目（已核实为正确，不修改）：
#   • #3 数组匹配 [[ " ${arr[*]} " =~ " $p " ]]：" $p " 两侧有空格，不会误匹配子串
#   • #12 IPv6 正则 fd00::/64：[0-9a-fA-F:]* 允许零字符，实测已正确匹配
#   • #14 detect_existing_hop_rules 未调用：main() 和 add_hop_interactive 均有调用
# ══════════════════ v5.1 修复（Gemini 报告）═══════════════════════════════
# [C1] apply_rules: 移除 daemon.json iptables=false 设置
#      原因：iptables=false 会停止 Docker 创建内核 DNAT，容器 -p 映射受影响
#      修复：不干预 daemon.json；docker-proxy 处理入站，masquerade 处理出站
# [C2] add_hop_interactive: nft list ruleset → nft -s list ruleset
#      [此项已随 C3 整体重构一并解决，不再有 nft list ruleset dump]
# [C3] add_hop_interactive: 移除 nft insert/add rule 增量插入
#      原因：nft insert 默认插到链顶，跳跃 accept 先于 ct invalid drop，
#            无效伪造包可绕过安全过滤（安全漏洞）
#      修复：新规则写入 HOP_RULES 内存，调用 apply_rules + save_rules 原子重载
# [C4] _get_docker_subnets: 补充 IPv6 子网正则
#      原因：只匹配 IPv4 格式，Docker IPv6 容器出站缺少 ip6 saddr masquerade
#      修复：grep 同时匹配 IPv4（n.n.n.n/）和 IPv6（含冒号）子网；
#            生成规则时按地址族分别用 ip/ip6 saddr masquerade
# [C5] SSH 防暴力破解: meter 废弃语法 → named dynamic set 现代写法
#      原因：meter 在 nftables v0.9.9+ 已废弃（Ubuntu 22.04/24.04 均超越此版）
#      修复：table 块内定义 set ssh_limit_v4/v6（type + flags dynamic + timeout）
#            chain 内用 add @set { key limit rate over ... } drop
# ══════════════════════════════════════════════════════════════════════════
...
SSH_PORT="" OPEN_PORTS=() HOP_RULES=() VERSION="5.5" DRY_RUN=false
...
# port.sh v5.5 (nftables) — 与 youhua.sh v2.4 / BBRplus 完全兼容
...
EOF
    ...
    # ── 内核版本检测（inet NAT 需要 ≥ 5.2）──────────────────
    # [E2] nft -c 能捕获语法错误，但内核过低时报错信息不直观
    # 在这里提前检测并给出明确提示，避免用户看不懂的 nft 错误
    local _kv
    _kv=$(uname -r | awk -F. '{print $1*1000+$2}')
    if [[ "$_kv" -lt 5002 ]]; then
        err "当前内核 $(uname -r) 不支持 inet NAT（需要 Linux ≥ 5.2）
  Ubuntu 22.04/24.04 已满足此要求；CentOS 7 / Debian 10 不支持"
    fi
    ok "内核版本 $(uname -r) ✓（≥ 5.2，支持 inet NAT）"
}
...
# ============================================================
# _get_docker_subnets: 获取所有 Docker 网络的 IPv4 和 IPv6 子网
# [C4 FIX] 补充 IPv6 格式（fd00::/80 等）
# ============================================================
_get_docker_subnets() {
...
# ============================================================
# _get_docker_bridges: 获取所有 Docker bridge 网络接口名
# [D1 FIX] Docker 自定义网络产生 br-xxxx 接口，而非只有 docker0
#   docker0    — 默认 bridge 网络
#   br-xxxxxx  — docker network create 产生的自定义网络
# 双路检测：docker network inspect 读取接口名 + ip link 直接枚举兜底
# ============================================================
_get_docker_bridges() {
    command -v docker &>/dev/null || { echo "docker0"; return; }

    # 路径一：通过 docker network inspect 读取 bridge 网络绑定的宿主机接口名
    docker network ls --format '{{.ID}}' 2>/dev/null \
        | while read -r nid; do
            local drv
            drv=$(docker network inspect "$nid" --format '{{.Driver}}' 2>/dev/null)
            [[ "$drv" == "bridge" ]] || continue
            local iface
            iface=$(docker network inspect "$nid" \
                --format '{{index .Options "com.docker.network.bridge.name"}}' \
                2>/dev/null)
            [[ -n "$iface" ]] && echo "$iface"
          done

    # 路径二：ip link 直接枚举宿主机上所有 docker0 / br-* 接口（兜底）
    # [E1 FIX] 原正则 [0-9a-f]+ 只匹配小写 hex，Docker 偶发大写（br-A1B2C3D4）
    # 改用 ip -o（单行输出）+ awk 切字段 + cut 去掉 @ifN 后缀，兼容所有大小写
    ip -o link show type bridge 2>/dev/null \
        | awk -F': ' '{print $2}' \
        | cut -d'@' -f1 \
        | grep -E '(docker[0-9]+|br-[0-9a-fA-F]+)' || true
}
...
#
# SSH 防暴力破解使用 nftables named dynamic set（现代写法）：
#   [C5 FIX] meter 已废弃，改为 named set + dynamic flag + timeout
#   set 定义在 table 块内，chain 内 add @set { key limit rate } drop
# ============================================================
build_nft_ruleset() {
    # ── 构建 nftables 端口集合字符串 ────────────────────────
    ...
    if [[ $_DOCKER_RUNNING -eq 1 ]]; then
        # [D1 FIX] 动态获取所有 Docker bridge 接口（docker0 + br-xxxx），
        # 而非只硬编码 docker0（自定义网络产生 br-xxxxxx 接口）
        local bridges
        mapfile -t bridges < <(_get_docker_bridges | sort -u)
        # 去重 + 兜底：确保 docker0 始终在列表中
        local seen_docker0=0
        for _b in "${bridges[@]:-}"; do [[ "$_b" == "docker0" ]] && seen_docker0=1; done
        [[ $seen_docker0 -eq 0 ]] && bridges+=("docker0")

        docker_forward_rules="        # Docker 容器网络转发（覆盖所有 bridge 接口）\n"
        for br in "${bridges[@]}"; do
            [[ -n "$br" ]] || continue
            docker_forward_rules+="        meta iifname \"${br}\" accept\n"
            docker_forward_rules+="        meta oifname \"${br}\" ct state { established, related } accept\n"
        done

        # postrouting masquerade：IPv4 用 ip saddr，IPv6 用 ip6 saddr
        local subnets
        mapfile -t subnets < <(_get_docker_subnets)
        if [[ ${#subnets[@]} -gt 0 ]]; then
            docker_masq_rules="        # Docker 出站 masquerade（IPv4 + IPv6）\n"
            for subnet in "${subnets[@]}"; do
                if [[ "$subnet" == *:* ]]; then
                    # IPv6 子网（含冒号）
                    docker_masq_rules+="        ip6 saddr ${subnet} masquerade\n"
                else
                    # IPv4 子网
            ...
            # 兜底：docker0 默认子网
            docker_masq_rules="        # Docker 出站 masquerade（默认 bridge 子网兜底）\n"
...
# ═════════════════════════════════════════════════════════════
# [G1 FIX] 原子替换模式（nftables batch 事务保证）：
#   1. 空声明（幂等 add，首次运行时建表，已有时无操作）
#   2. delete 删除旧表（含旧规则）
#   3. 重新声明完整规则
# 整个序列在同一 nft batch 内原子提交，无任何防御真空期；
# 语法错误时内核回滚，旧规则完整保留，服务器不会裸奔。
table inet filter {}
delete table inet filter
table inet filter {

    # ── SSH 防暴力破解：命名动态集合（现代写法）─────────────
    # [C5 FIX] meter 语法已废弃，改用 nftables 原生 dynamic set：
    #   - 按源 IP 独立计速，每条记录 60 秒后自动过期
    #   - add @set { key limit rate over X } 是 nftables 推荐写法
    #   - Ubuntu 22.04/24.04 均已内置，无需额外内核模块
    set ssh_limit_v4 {
        type ipv4_addr
        flags dynamic
        timeout 60s
    ...
    chain input {
        ...
        # ── SSH 防暴力破解（使用上方定义的 dynamic set）──────
        # 60 秒内同一源 IP 发起超过 10 次新连接则丢弃该 IP
        tcp dport ${SSH_PORT} ct state new \\
            add @ssh_limit_v4 { ip saddr limit rate over 10/minute burst 10 packets } drop
        tcp dport ${SSH_PORT} ct state new \\
            add @ssh_limit_v6 { ip6 saddr limit rate over 10/minute burst 10 packets } drop
...
NFT_EOF
    ...
    chain forward {
        ...
        # [F2 FIX] FORWARD 链改为 policy accept（与 Docker 共存的正确姿势）
        #
        # 原因：当外部包命中 Docker 的 DNAT 规则（ip nat PREROUTING），目标 IP
        # 变为容器内网地址，内核路由判定为"非本机"，包走 FORWARD 而非 INPUT。
        # 若 policy drop，所有 docker -p 映射端口的入站新连接全部被拦截。
        #
        # 安全说明：policy accept 并不等于"不设防"——
        #   • Docker 的 iptables-nft 规则（ip filter FORWARD）依然生效，
        #     它管理容器间隔离与出入站过滤（由 Docker 维护，我们不干预）
        #   • 本脚本的 inet filter INPUT（policy drop + 白名单）继续保护本机
        #   • 纯路由/转发流量（非 Docker）在此环境极少见，如需限制可手动加规则
        ct state invalid drop
        ct state { established, related } accept
NFT_EOF
...
apply_rules() {
    ...
    # ── 检测 Docker ───────────────────────────────────────────
    # [C1 FIX] 不再设置 daemon.json iptables=false。
    #
    # 原因：强制 iptables=false 会让 Docker 停止为 "-p 宿主机:容器" 创建内核 DNAT 规则。
    # 虽然 docker-proxy 仍在用户态转发，但这依赖 docker-proxy 未被禁用，
    # 行为不一致且会使用户困惑（面板容器端口映射看似生效却不稳定）。
    #
    # 正确方案：
    #   • 入站（-p 映射）：docker-proxy 进程绑定宿主机端口，ss -tulnp 可扫描到，
    #     本脚本在 detect_ports 中自动将这些端口加入 OPEN_PORTS 放行，无需干预。
    #   • 出站（容器→外网）：本脚本在 nftables postrouting 生成 masquerade 规则覆盖。
    #   • 转发：本脚本在 forward 链放行 docker0 进出流量。
    #   • Docker 自身的 iptables-nft 规则（ip 族）与本脚本的规则（inet 族）在
    #     nf_tables 内核层面是独立的不同 family，互不干扰。
    _DOCKER_RUNNING=0
    if command -v docker &>/dev/null && docker info &>/dev/null 2>&1; then
        _DOCKER_RUNNING=1
        info "检测到 Docker 正在运行，将在 nftables 中生成 masquerade 和转发规则..."
        info "Docker 容器 -p 映射端口已由 ss 扫描自动加入放行列表，无需修改 daemon.json"
    fi
    ...
    # [G1 FIX] 删表操作移入 .nft 文件内部（declare→delete→redeclare 原子模式）
    # bash 层不再提前删表，彻底消除两个风险：
    #   • 裸奔风险：nft -c 失败时 bash 已删表 → 服务器无防火墙
    #   • 真空期：bash delete 到 nft -f 之间有毫秒级无规则窗口
    # nft -c 只做语法解析不执行，nft -f 将整个文件作为单一内核 batch 原子提交
...
reset_fw() {
    ...
    # [D2] 精准删除本脚本的两张表，不触碰 fail2ban/k8s 等其他 nft 表
    nft delete table inet filter 2>/dev/null || true
    nft delete table inet nat    2>/dev/null || true
    ok "nftables 规则集已清空（全部放行，其他 nft 表不受影响）"

    # 保存空规则集（纯 nftables 语法，无 shell 操作符）
    # [F1 FIX] 空配置文件只声明注释，不写任何操作；
    # 两张表已在上方用 nft delete 删除，重启后 nftables.service
    # 加载此空文件，结果是无规则（全部放行）
    {
        printf '#!/usr/sbin/nft -f
'
        printf '# port.sh v%s — 已重置为全部放行
' "${VERSION}"
        printf '# %s
' "$(date '+%Y-%m-%d %H:%M:%S')"
        printf '# nftables 表已清空，全部放行中
'
    } > /etc/nftables.conf
...
# [C3 FIX] 不再采用增量 nft add rule，改为原子重载完整规则集
# ============================================================
add_hop_interactive() {
    detect_ssh
    ...
    # ── [C3 FIX] 重新原子加载，而非增量 nft insert ──────────
    # 原因: nft insert 默认插到链顶，会把跳跃端口的 accept 置于
    #       ct state invalid drop 之前，无效伪造包得以绕过安全过滤。
    # 正确做法：将新规则加入内存状态，调用 apply_rules 重新生成
    # 一份顺序正确的完整规则集并原子加载，不存在规则错位问题。

    # 1. 从运行中的 nftables 读取已有跳跃规则（确保内存与现实同步）
    detect_existing_hop_rules

    # 2. 将新跳跃规则加入内存
    local new_rule="${s}-${e}->${target_port}"
    if [[ ${#HOP_RULES[@]} -eq 0 ]] || \
       [[ ! " ${HOP_RULES[*]} " =~ " ${new_rule} " ]]; then
        HOP_RULES+=("$new_rule")
    fi
    # 目标端口也需要放行（作为代理实际监听端口）
    add_port "$target_port"

    # 3. 补充当前公网监听端口（确保 ss 扫描结果不丢失）
    while read -r port; do
        add_port "$port"
    done < <(get_public_ports)
    add_port 80
    ...
    # 4. 原子加载 + 持久化（apply_rules 内部会自行检测 Docker 状态）
    # [F3 FIX] 移除冗余的 Docker 检测，apply_rules 首行已有完整检测逻辑
    apply_rules
    save_rules
...
show_summary() {
    ...
    if [[ $_DOCKER_RUNNING -eq 1 ]]; then
        echo -e "  ${G}▸ Docker 检测到，-p 映射端口已由 ss 扫描自动放行${W}"
        echo -e "  ${G}▸ 容器出站 masquerade 已写入 nftables postrouting${W}"
        echo -e "  ${G}▸ daemon.json 未修改，Docker iptables-nft 规则正常共存${W}"
    fi
