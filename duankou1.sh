...
add_port_range_interactive() {
    ...
    while true; do
        ...
        # 检查格式
        if echo "$port_range" | grep -qE '^[0-9]+-[0-9]+
...
detect_listening_ports() {
    ...
    while IFS= read -r line; do
        # 跳过标题行
        if echo "$line" | grep -qE '^(Netid|State)'; then
            continue
        fi
        
        # 只处理 LISTEN 或 UNCONN 状态
...
# 执行主函数
main "$@"; then
                if [ "$target" -ge 1 ] && [ "$target" -le 65535 ]; then
                    NAT_RULES+=("$start-$end->$target")
                    DETECTED_PORTS+=("$target")
                    success "添加: $start-$end -> $target"
                    
                    read_with_timeout "${YELLOW}继续添加？[y/N]${RESET}" 10 "N"
                    if echo "$REPLY" | grep -qv '^[Yy]
