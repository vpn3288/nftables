...
RESET="\033[0m"
脚本信息
SCRIPT_VERSION="2.2.1"  # 更新版本以表示完善
SCRIPT_NAME="精确代理端口防火墙管理脚本（nftables 版本）"
SCRIPT_FILE="duankou1.sh"  # 新增：固定脚本文件名，用于管理命令
...
echo -e "${CYAN}💡 提示: 如需重新配置端口转发，请运行 'bash $SCRIPT_FILE --add-range'${RESET}"
fi
}
解析参数
parse_arguments() {
...
local port="$1"
for common_port in "${STANDARD_PROXY_PORTS[@]}" "${HIDDIFY_COMMON_PORTS[@]}"; do
if [ "$port" = "$common_port" ]; then
return 0
fi
done
if [ "$port" -ge 10000 ] && [ "$port" -le 65535 ] && ! is_internal_service_port "$port"; then
return 0  # 扩展：认为10000+端口可能是代理端口
fi
return 1
}
端口安全检查
...
echo -e " ${YELLOW}重新配置:${RESET} bash $SCRIPT_FILE"
echo -e " ${YELLOW}重置防火墙:${RESET} bash $SCRIPT_FILE --reset"
echo -e " ${YELLOW}添加端口转发:${RESET} bash $SCRIPT_FILE --add-range"
echo -e " ${YELLOW}清理NAT规则:${RESET} bash $SCRIPT_FILE --clean-nat"
}
显示最终状态
show_final_status() {
echo -e "\n${GREEN}=================================="
...
echo -e " ${YELLOW}查看状态:${RESET} bash $SCRIPT_FILE --status"
echo -e " ${YELLOW}添加端口转发:${RESET} bash $SCRIPT_FILE --add-range"
echo -e " ${YELLOW}清理NAT规则:${RESET} bash $SCRIPT_FILE --clean-nat"
echo -e " ${YELLOW}重置防火墙:${RESET} bash $SCRIPT_FILE --reset"
